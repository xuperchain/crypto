...
func RandomGenerate(degree int, secret []byte) ([]*big.Int, error) {
	...
	//	byteSlice[0] = intercept
	result[0] = intercept

	// 生成非最高次方位的随机参数
	// TODO 需要重构，多个bytes组成一个bigint
	if degree > 1 {
		randomBytes, err := rand.GenerateSeedWithStrengthAndKeyLen(rand.KeyStrengthHard, degree-1)
		if err != nil {
			return nil, err
		}

		for i := 0; i < len(randomBytes); i++ {
			byteSlice[0] = randomBytes[i]
	...
	for {
		randomByte, err := rand.GenerateSeedWithStrengthAndKeyLen(rand.KeyStrengthHard, 1)
		if err != nil {
			return nil, err
		}

		highestDegreeCoefficient := big.NewInt(0).SetBytes(randomByte)
